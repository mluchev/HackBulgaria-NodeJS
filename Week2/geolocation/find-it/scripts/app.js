...
require(['jquery','async!http://maps.google.com/maps/api/js?sensor=false', 'jquery-color'], function($) {
    ...
    function getLocations() {
        ...
        $.ajax({
            type: "GET",
            url: 'http://localhost:4444/locations',
            headers: { 'query-json-header' : JSON.stringify(data) },
            success: function(resultPoints) {
                showPoints(resultPoints);
    ...
    function showPoints(points) {
        ...
        points.forEach(function(point) {
            var latLng = new google.maps.LatLng(point.position.coordinates[1], point.position.coordinates[0]);
    ...
    function initializeLargeMap() {
        var mapOptions = {
            zoom: 5,
            center:  new google.maps.LatLng(42.63822186937074, 23.384056091308594)
        };
